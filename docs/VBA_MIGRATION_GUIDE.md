...
- `Módulo12.bas` — `EMAIL`, `Worksheet_Change` ✅ **substituído** (2026-07-17/20) — não é um port literal do e-mail Outlook, é uma solução web-nativa melhor: a fila do setor Cadastro (`CadastroPage.tsx`, `/cadastro`) recebe todo BITin enviado; quando precisa de revisão de roteiro, encaminha pro setor Processos dentro do próprio sistema. Ver `docs/BITIN_MODEL.md`, seção "Roteamento pós-envio (Cadastro → Processos)", e `docs/releases/RELEASE_v0.9.0.md`.
