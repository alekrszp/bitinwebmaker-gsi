...
| POST | `/users/{id}/resetar-senha` | "Esqueci minha senha" (2026-07-21) — exige `NIVEL_ADMIN` (99), na prática só o super-admin oculto. Sem corpo — gera senha temporária nova (mesmo padrão de `reativar`, devolvida uma única vez), sem mexer em e-mail/`ativo`. 400 se o alvo está excluído (reative a conta primeiro). Existe porque o backend não tem SMTP configurado — um fluxo self-service por e-mail não teria como entregar nada de verdade, então virou uma ação do admin dentro de Gestão de usuários em vez disso. |
